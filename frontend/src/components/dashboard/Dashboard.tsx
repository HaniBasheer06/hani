...
export function Dashboard() {
  ...
  const yearRange = summary.data?.year_range ?? [];
  ...
  return (
    <main className="app-shell">
      ...
      <section className="hero-row">
        <div><p className="eyebrow">NATIONAL RESOURCE INTELLIGENCE / {yearRange[yearRange.length - 1] ?? 'AVAILABLE DATA'}</p><h1>Mining operations, <em>mapped.</em></h1><p className="hero-copy">A decision-support surface for India&apos;s mineral production, terrain signals, and feasibility intelligence.</p></div>
      ...
      {summary.isLoading ? <LoadingBlock /> : summary.isError ? <ErrorState message="Summary data could not be loaded." /> : <section className="kpi-grid">
        ...
        <Kpi icon={<BarChart3 />} label="DATA HORIZON" value={yearRange.length ? `FY ${yearRange[0]} — ${yearRange[yearRange.length - 1]}` : 'N/A'} suffix={`${number.format(summary.data?.training_rows ?? 0)} ML rows · ${number.format(summary.data?.synthetic_training_rows ?? 0)} augmented`} />
      </section>}
